...
function aggiorna() {
    document.getElementById('containerConti').innerHTML=''
    for (i = 1; i <= contaConti; i++) {
        if (localStorage.getItem('conto' + i) != null) {
            ...
            let contoHTML = '<div class="border border-danger w-50 h-50 h2 align-self-center p-5" id="conto' + i + '">' +
                '<h3 class="m-2 text-center text-secondary" id="titoloConto' + i + '">Account ' + i + ' <span id="inUso' + i + '">(Using)</span>' + '</h3>' +
                '<p class="m-2 text-center">€' + localStorage.getItem(conto) + ',00</p>' +
                '<div class="d-flex flex-row justify-content-center p-2 gap-4">' +
                '<button class="btn btn-danger btn-xl p-3" id="' + i + '" onclick="rimuoviConto(this.id);" href="#">Delete Account</button>' +
                '<button class="btn btn-primary btn-xl p-3" id="' + i + '" onclick="selezionaConto(this.id);" href="#">Select Account</button>' +
                '</div>' +
                '</div>';
            ...
            if (contoSelezionato == i) {
                ...
                document.getElementById('inUso' + i).style.display = "inline";
            } else {
...
function aggiungiConto() {
    contaConti++;
    let soldi = prompt("Type in initial balance: "); //soldi presenti sul conto che si vuole aggiungere

    let conto = '<div class="border border-danger w-50 h-50 h2 align-self-center p-5" id="conto' + contaConti + '">' +
        '<h3 class="m-2 text-center text-primary" id="titoloConto' + contaConti + '">Account ' + contaConti + ' <span id="inUso' + contaConti + '">(Using)</span>' + '</h3>' +
        '<p class="m-2 text-center">€' + soldi + ',00</p>' +
        '<div class="d-flex flex-row justify-content-center p-2 gap-4">' +
        '<button class="btn btn-danger btn-xl p-3" id="' + contaConti + '" onclick="rimuoviConto(this.id);" href="#">Delete Account</button>' +
        '<button class="btn btn-primary btn-xl p-3" id="' + contaConti + '" onclick="selezionaConto(this.id);" href="#">Select Account</button>' +
        '</div>' +
        '</div>';
...
function rimuoviConto(nConto) {
    if (contoSelezionato == nConto) {
        alert("You're currently using this account!")
        return
    } //se il conto attualmente in uso viene eliminato
    for (let i = nConto; i <= contaConti; i++) {
        if(contoSelezionato==i) {
            localStorage.setItem('contoSelezionato', parseInt(i)-1)
            contoSelezionato = localStorage.getItem('contoSelezionato'); //conto attualmente in uso
        }
        if (i < contaConti) {
            soldi = localStorage.getItem('conto' + (parseInt(i) + 1))
            localStorage.setItem('conto' + i, soldi)
            document.getElementById('conto'+i).remove();
